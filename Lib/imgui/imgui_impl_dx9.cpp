...
void* ImGui_CreateTextureRGBA(int width, int height, const unsigned char* data)
{
    ...
    ImGui_ImplDX9_Data* bd = ImGui_ImplDX9_GetBackendData();
    if (bd->pd3dDevice->CreateTexture(width, height, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM, &tempTexture, nullptr) != D3D_OK)
        return nullptr;
    ...
    if (bd->pd3dDevice->CreateTexture(width, height, 1, D3DUSAGE_AUTOGENMIPMAP, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &texture, nullptr) != D3D_OK) {
        tempTexture->Release();
        return nullptr;
    }

    bd->pd3dDevice->UpdateTexture(tempTexture, texture);
    tempTexture->Release();
