...
///-- License       MIT
///--------------------------------------------------------------------------------
///-- Will clean that code when i will have time and motivation xs9 ;///
///--------------------------------------------------------------------------------
#ifndef BSPPARSERH
#define BSPPARSERH
	...
	private:
		...
		)
		{
			...
			if (num_leaffaces > valve::MAX_MAP_LEAFBRUSHES) {
				//cmd.AddLog(("[!] map has to many leaffaces, parsed more than required...\n")); 
			}
			else if (!num_leaffaces) {
				//cmd.AddLog(("[!] map has no leaffaces to parse...\n")); 
			}
		...
		)
		{
			...
			const auto num_leaffaces = _leaf_faces.size(); 
			if (num_leaffaces > valve::MAX_MAP_LEAFBRUSHES) {
				//cmd.AddLog(("[!] map has to many leafbrushes, parsed more than required...\n")); 
			}
			else if (!num_leaffaces) {
				//cmd.AddLog(("[!] map has no leafbrushes to parse...\n")); 
			}
		...
		)
		{
			...
			if (start_distance >= 0.f && end_distance >= 0.f) {
				rayCastNode(node->children.at(0), start_fraction, end_fraction, origin, destination, out); 
			}
			else if (start_distance < 0.f && end_distance < 0.f) {
				rayCastNode(node->children.at(1), start_fraction, end_fraction, origin, destination, out); 
			}
			else {
				std::int32_t side_id; 
				...
				rayCastNode(node->children.at(side_id), start_fraction, fraction_middle, origin, middle, out); 
				...
				rayCastNode(node->children.at(!side_id), fraction_middle, end_fraction, middle, destination, out); 
			}
		}
		...
		) const
		{
			if (brush->num_sides) {
				...
				for (auto i = 0; i < brush->num_sides; ++i) {
					...
					if (start_distance > end_distance) {
						auto fraction = _max((start_distance - valve::DIST_EPSILON), 0.f); 
	...
	public:
		...
		)
		{
			...
			try {
				...
				//cmd.AddLog(("BSP version %d\n"), _bsp_header.version); 

				if (!valve::has_valid_bsp_ident(_bsp_header.ident)) {
					//cmd.AddLog(("[!] %s isn't a (valid) .bsp file!\n"), map_name.data()); 
					return false; 
				}
		...
		bool isVisible(const Vector& origin, const Vector& destination)
		{
		...
		)
		{
			if (!_planes.empty() && out) {
				...
				rayCastNode(0, 0.f, 1.f, origin, final, out);
